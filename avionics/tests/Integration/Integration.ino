...
const int16_t Gyro_Drift_Threshold = 1; // 10/14.135
...
void loop() {
  while (!isLaunch) {
    ...
    // Calculate new gyro offsets and check for threshold
    ...
    if (abs(_accelX) > launchAccel) {
      ...
      dataBuffer[bufferIndex++] = 0b01010100; // ID and length for gyro data
      ...
      SerialFlash.write(pageAddr, dataBuffer, bufferIndex); // Write only the gyro data
