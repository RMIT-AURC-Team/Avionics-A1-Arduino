...
void getGyroscopeData(int *result) {
  ...
  *************************************/
  ...
  result[0] = ((buff[2] << 8) | buff[3]) +
              g_offx; // they are 2 byte registers within the sensor
...
void setup() {
  ...
  // accel - we can go through if and delete a lot of these as they are to do
  // with tap sensing
  adxl.powerOn();
...
const unsigned long highResolutionInterval =
    2000;  // Interval for high resolution (500Hz) in microseconds (1000000
           // microseconds / 500Hz)
const unsigned long lowResolutionInterval =
    20000; // Interval for low resolution (50Hz) in microseconds (1000000
           // microseconds / 50Hz)
...
void loop() {
  ...
  if (currentMicro - previousHighResolutionMicro >= highResolutionInterval) {
    ...
    adxl.readXYZ(
        &x, &y,
        &z); // read the accelerometer values and store them in variables  x,y,z
    ...
    hx = gyro[0]; // 14.375; // this is the scale within the datasheet we will
    ...
    // ------------------------------------------------------------------------
    int16_t mx, my, mz, mt;
    compass.readRaw(&mx, &my, &mz,
                    &mt); // reads the raw data not sure exactly what the scale
                          // is at all, we need comb the data sheet
    ...
    uint8_t syncMillis =
        currentMicro /
        1000; // Convert microseconds to milliseconds and truncate
    ...
    dataBuffer[BufferIndex++] =
        (uint8_t)((accel_x >> 8) & 0xFF); // Store the high byte of accel_x
    dataBuffer[BufferIndex++] =
        (uint8_t)(accel_x & 0xFF);        // Store the low byte of accel_x
    dataBuffer[BufferIndex++] =
        (uint8_t)((accel_y >> 8) & 0xFF); // Store the high byte of accel_y
    dataBuffer[BufferIndex++] =
        (uint8_t)(accel_y & 0xFF);        // Store the low byte of accel_y
    dataBuffer[BufferIndex++] =
        (uint8_t)((accel_z >> 8) & 0xFF); // Store the high byte of accel_z
    dataBuffer[BufferIndex++] =
        (uint8_t)(accel_z & 0xFF);        // Store the low byte of accel_z

    dataBuffer[BufferIndex++] =
        (uint8_t)((hx >> 8) & 0xFF); // Store the high byte of hx
    dataBuffer[BufferIndex++] =
        (uint8_t)(hx & 0xFF); // Store the low byte of hx
    dataBuffer[BufferIndex++] =
        (uint8_t)((hy >> 8) & 0xFF); // Store the high byte of hy
    dataBuffer[BufferIndex++] =
        (uint8_t)(hy & 0xFF); // Store the low byte of hy
    dataBuffer[BufferIndex++] =
        (uint8_t)((hz >> 8) & 0xFF); // Store the high byte of hz
    dataBuffer[BufferIndex++] =
        (uint8_t)(hz & 0xFF); // Store the low byte of hz

    dataBuffer[BufferIndex++] =
        (uint8_t)((magnetometer_x >> 8) &
                  0xFF); // Store the high byte of magnetometer_x
    dataBuffer[BufferIndex++] =
        (uint8_t)(magnetometer_x &
                  0xFF); // Store the low byte of magnetometer_x
    dataBuffer[BufferIndex++] =
        (uint8_t)((magnetometer_y >> 8) &
                  0xFF); // Store the high byte of magnetometer_y
    dataBuffer[BufferIndex++] =
        (uint8_t)(magnetometer_y &
                  0xFF); // Store the low byte of magnetometer_y
    dataBuffer[BufferIndex++] =
        (uint8_t)((magnetometer_z >> 8) &
                  0xFF); // Store the high byte of magnetometer_z
    dataBuffer[BufferIndex++] =
        (uint8_t)(magnetometer_z &
                  0xFF); // Store the low byte of magnetometer_z
    dataBuffer[BufferIndex++] =
        (uint8_t)((magnetometer_t >> 8) &
                  0xFF); // Store the high byte of magnetometer_t
    dataBuffer[BufferIndex++] =
        (uint8_t)(magnetometer_t &
                  0xFF); // Store the low byte of magnetometer_t
  ...
  if (currentMicro - previousLowResolutionMicro >= lowResolutionInterval) {
    ...
    dataBuffer[BufferIndex++] =
        (uint8_t)((praw >> 16) & 0xFF); // Store the first byte of praw
    dataBuffer[BufferIndex++] =
        (uint8_t)((praw >> 8) & 0xFF); // Store the second byte of praw
    dataBuffer[BufferIndex++] =
        (uint8_t)(praw & 0xFF); // Store the third byte of praw

    dataBuffer[BufferIndex++] =
        (uint8_t)((traw >> 16) & 0xFF); // Store the first byte of traw
    dataBuffer[BufferIndex++] =
        (uint8_t)((traw >> 8) & 0xFF); // Store the second byte of traw
    dataBuffer[BufferIndex++] =
        (uint8_t)(traw & 0xFF); // Store the third byte of traw
  ...
  if (counter == 4) {
    ...
    // Read and print data for each page
    for (int i = 0; i < 5; ++i) {
